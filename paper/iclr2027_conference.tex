...
LeWorldModel (LeWM)~\citep{lewm2026} enables reward-free visual goal
reaching by jointly learning an encoder and an action-conditioned latent
predictor from offline trajectories. At test time, LeWM uses the
cross-entropy method (CEM)~\citep{rubinstein1999cem} to optimize action
sequences: sampled candidates are rolled out through the latent predictor
and ranked by the distance between their predicted endpoints and a goal
image.
...
Reward-free visual goal reaching uses latent dynamics to evaluate
image goals specified after training. PLDM learns these dynamics
...
Longer-range planning can extend prediction or structure search.
Variable-length world models condition on action sequences of
different durations~\citep{du2026vlwm}, and hierarchical world
models use coarse predictions to guide local
planning~\citep{zhang2026hwm}. Subgoal methods include FF-JEPA,
which proposes successive subgoals with an action-free latent
planner~\citep{masip2026ffjepa}, and SAGE, which conditions action
generation on predicted subgoals~\citep{cheng2026sage}. These
methods extend available futures or guide action proposals.

Planning-oriented objectives also shape endpoint evaluation.
Temporal straightening improves latent distance geometry by
regularizing trajectory curvature~\citep{wang2026straightening};
RC-aux combines multi-horizon prediction with budget-conditioned
reachability correction~\citep{li2026rcaux}. Traj-LeWM supplements
endpoint distance with a goal-conditioned trajectory
cost~\citep{huang2026trajlewm}, and TRM learns a temporal pairwise
cost for ranking endpoints against goals~\citep{li2026trm}.
HAS instead advances each LeWM candidate endpoint with a shared
action-free predictor, setting recursion depth from the remaining
goal gap. It retains Euclidean goal distance and LeWM's CEM
proposal and update rules.
