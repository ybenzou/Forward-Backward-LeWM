...
The exception is TwoRoom at $o=100$: fixed $k=5$ achieves
$76.2\%$, compared with $70.4\%$ for HAS. The preferred depth
can thus be task-dependent, with the remaining-gap rule providing
a strong cross-condition default. At $o=25$, the $k=0$ cost
identity (Section~\ref{sec:alignment}) corresponds to matching
success in TwoRoom and Reacher and near-matching success in
PushT ($87.2\%$ for LeWM and $87.0\%$ for HAS).
...
observed advantage rather than uniform success rates. At $o=25$,
HAS uses $k=0$, and the separately trained checkpoints give similar
success rates without a systematic gain. Because the LeWM reference
is fixed, this experiment does not establish training-seed-matched
LeWM--HAS improvements.
...
The diagnostic exposes the transformation of observed endpoint
distances, while closed-loop comparisons establish the control
benefit. Appendix~\ref{app:analysis} gives the sampling protocol.
...
The paired comparisons use one training checkpoint per task and
ten evaluation groups. The three-seed analysis varies HAS training
against a fixed LeWM reference, without matched LeWM training runs.
The $H=50$ control examines
