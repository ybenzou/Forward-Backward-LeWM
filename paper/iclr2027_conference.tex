...
visual goal. When the goal lies beyond this span, however, direct
endpoint scoring can become temporally myopic and undervalue actions
whose benefits emerge through later execution. We introduce
Horizon-Aligned Scoring (HAS), which learns an action-free latent
predictor from local transitions in LeWM's existing reward-free
trajectories and applies it recursively to extend each action-conditioned
endpoint to the goal's time scale before scoring. The recursion depth
follows the remaining temporal gap and decreases during execution.
Across three training seeds per task and
ten paired evaluation seeds per trained model on PushT, TwoRoom, and
Reacher, HAS improves mean success at every tested goal distance beyond
LeWM's default $25$-step planning span, with particularly large gains
for goals $50$ steps ahead in PushT and $100$ steps ahead in TwoRoom.
HAS recovers the default LeWM scorer when no extension is needed and
outperforms the evaluated $50$-step LeWM configuration at every longer
goal distance. Fixed-depth comparisons show that no constant extension
depth performs consistently across conditions. These results demonstrate
that aligning the temporal reach of endpoint scoring with the goal
horizon mitigates temporal myopia in goal-conditioned world-model
planning.
\end{abstract}
...
without lengthening the action-conditioned search. Third, experiments
across three visual-control benchmarks spanning object manipulation,
navigation, and reaching, with three training seeds per task and ten
paired evaluation seeds per trained model, show that HAS improves mean
success at every tested goal distance beyond LeWM's default planning
span. Comparisons with a tested longer-span planner and fixed extension
depths further show that neither simply increasing the action horizon
nor applying a constant amount of temporal extension can consistently
reproduce these gains.
