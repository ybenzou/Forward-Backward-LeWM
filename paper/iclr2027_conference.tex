...
examples for $F$: $F(p_0)$ predicts $z_2$ and $F(p_1)$
...
trains $F$ to consume its own output over two transitions.
Both draw supervision from the same four-observation sample, requiring
no additional trajectories or targets at the evaluated goal offsets.

Inputs and targets are detached at the interface to $F$,
