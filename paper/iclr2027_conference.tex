...
predictor from local transitions and applies it recursively to extend
each action-conditioned endpoint to the goal's time scale before
scoring. The recursion depth follows the remaining temporal gap and
decreases during execution. Across three training seeds per task and
...
action-free latent predictor, learned from local latent transitions,
then recursively advances that endpoint before it is compared with the
goal. One-step and recursive supervision support repeated application.
During planning, the remaining temporal gap determines the recursion
depth, which decreases as execution advances. HAS thereby extends the
temporal reach of candidate scoring while leaving the CEM sampling and
update rules, as well as the candidate action horizon, unchanged.
...
HAS, which recursively extends each candidate endpoint with an
action-free latent predictor and adapts the extension depth to the
remaining temporal gap without lengthening the action-conditioned
search. Third, experiments on PushT, TwoRoom, and Reacher across three
training seeds per task and ten paired evaluation seeds per trained
model show that HAS improves mean success at every tested goal distance
beyond LeWM's default planning span. Comparisons with a tested longer-span
planner and fixed extension depths further show that neither simply
increasing the action horizon nor applying a constant amount of temporal
extension can consistently reproduce these gains.
