...
HAS instead focuses on the temporal representation supplied to the
endpoint comparison. Its recursive action-free latent prediction is
related to latent subgoal prediction~\citep{masip2026ffjepa}, while its
use for ranking action-conditioned endpoints relates to
planning-oriented costs~\citep{li2026trm,huang2026trajlewm}. Unlike
methods that propose subgoals or learn a new comparison function, HAS
applies a shared latent predictor after each candidate rollout, selects
the recursion depth from the remaining gap to the goal, and retains
Euclidean goal distance. This gives each short endpoint a longer-horizon
scoring view without changing the CEM proposal or update rules.
...
\begin{table}[!htbp]
\caption{Success rates (\%) across HAS training runs. The LeWM reference
uses one checkpoint and ten evaluation groups; each HAS seed uses its
own checkpoint and the same ten groups. The final column reports
mean $\pm$ sample standard deviation across the three HAS run means.
The LeWM reference is not training-seed matched.}
...
An evaluation-disjoint diagnostic over $400$ windows per task shows
that recursive extension generally lowers endpoint-to-goal latent
distance. The direct and extended distributions overlap more for
PushT at the longest offset. Appendix~\ref{app:analysis} gives the
figure, interpretation, and sampling protocol.
...
\section{Scoring diagnostic}
\label{app:analysis}

Temporal extension makes intermediate states more comparable to
later goals. We examine this transformation directly on encoder
endpoints, comparing $\|z_H-z_g\|_2$ with
$\|F^k(z_H)-z_g\|_2$ over $400$ evaluation-disjoint windows per
task. We use $H=25$ and $(k,o)\in\{(5,50),(10,75),(15,100)\}$.

\begin{figure}[!htbp]
\centering
\includegraphics[width=0.95\linewidth]{figures/fig_has_score_contrast.pdf}
\caption{Temporal extension of observed endpoint distances:
$\|z_H-z_g\|_2$ versus $\|F^k(z_H)-z_g\|_2$ on $400$
evaluation-disjoint windows per task ($H=25$).
Axes show Euclidean distance and window count.}
\label{fig:score}
\end{figure}
\FloatBarrier
...
benefit. Paired executions connect this scoring perspective to
intermediate progress during control.
