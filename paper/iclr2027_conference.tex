...
Visual world models provide a reusable interface for goal-conditioned
control: they encode observations, predict the effects of candidate
actions, and evaluate those predictions against a goal image.
Learning this interface from reward-free trajectories allows the same
model to support goals specified after
training~\citep{sobal2025rewardfree,zhou2025dinowm,lewm2026}.
The resulting controller depends on two complementary components:
a model of local dynamics and a scoring rule that identifies which
predicted outcomes constitute useful progress. A short, accurate
forecast becomes useful for a distant goal through the quality of
that second component.

This distinction matters when the goal lies beyond the planning span.
LeWorldModel (LeWM)~\citep{lewm2026}, an end-to-end Joint-Embedding
Predictive Architecture (JEPA)~\citep{lecun2022jepa}, ranks action
sequences by the distance between their predicted endpoints and the
goal embedding. Under its default $25$-step planning span, the
endpoint can represent an intermediate stage of a much longer task.
For example, movement toward a passage can be useful for reaching
another room even while the current plan ends far from the goal.
Direct terminal distance asks a representation of this intermediate
stage to stand in for its later consequences. We frame this
\emph{temporal mismatch} as a limitation of terminal scoring and
seek a longer-term view of each candidate's endpoint.

Horizon-Aligned Scoring (HAS) supplies this view through a learned
temporal extension (Figure~\ref{fig:overview}).
The action-conditioned predictor first rolls out the candidate
sequence over the short planning span. An action-free Forward
imaginer then recursively advances its endpoint, and the resulting
representation is compared with the goal. Forward learns local
latent transitions from the same observation sequences used to train
the world model, with supervision on both one-step predictions and
their two-step composition. At evaluation, its recursion depth
reflects the remaining time between the short rollout and the goal,
so the extension contracts as the controller makes progress.
...
We test whether this extension improves long-offset goal reaching
and whether its depth should follow the remaining temporal gap.
Across ten paired evaluation groups on PushT, TwoRoom, and Reacher,
HAS improves mean success in every evaluated condition beyond the
default span. The gains include $42.8\%$ to $67.8\%$ on PushT at
offset $50$ and $16.4\%$ to $70.4\%$ on TwoRoom at offset $100$.
A LeWM configuration with a doubled planning span remains below HAS
in these long-offset comparisons. Applying the same Forward map at
fixed depths further shows that its usefulness depends on how the
extension evolves during execution.

Our contribution is a temporal perspective on the scoring interface
between short-horizon prediction and longer-horizon control.
HAS instantiates this perspective with a locally trained Forward map
and a remaining-gap recursion rule integrated into receding-horizon
planning. The experiments establish the practical value of this
scoring design and identify depth selection as a consequential part
of its use. This perspective connects work on extending latent
dynamics with work on making latent representations useful for
planning.

\section{Related Work}
\label{sec:related}

Latent world models make visual control tractable by representing
future evolution in a compact state space. PlaNet performs online
planning through learned latent dynamics~\citep{hafner2019planet};
Dreamer learns behavior through latent
imagination~\citep{hafner2020dreamer}; and TD-MPC combines short model
rollouts with a learned terminal value~\citep{hansen2022tdmpc}.
Reward-free visual goal reaching places a different demand on this
interface: the controller must evaluate progress toward a newly
specified observation. PLDM studies this setting through latent
dynamics learned from offline trajectories~\citep{sobal2025rewardfree}.
DINO-WM predicts pretrained visual
features~\citep{zhou2025dinowm}, while LeWM jointly learns the encoder
and predictor with predictive and distributional
objectives~\citep{lewm2026}. Together these approaches establish
latent prediction as a basis for goal-conditioned control and
motivate the question of how far into the future its scoring rule
should look.

Extending temporal coverage can improve the futures available to
a planner. Variable-length latent world models predict outcomes
conditioned on action sequences of different
durations~\citep{du2026vlwm}. Hierarchical world models share a latent
space across temporal resolutions and use coarse predictions to
guide local planning~\citep{zhang2026hwm}. This decomposition also
supports learned subgoal interfaces: FF-JEPA uses an action-free
latent planner to propose successive
subgoals~\citep{masip2026ffjepa}, and SAGE uses predicted subgoals to
condition candidate action generation~\citep{cheng2026sage}.
These methods organize longer-term prediction or guide the search
toward useful local plans. Once a candidate has been generated, a
complementary opportunity is to evaluate its endpoint over the
time scale of the remaining task.

The geometry and training of the evaluation signal are also central
to effective planning. Temporal straightening regularizes latent
trajectory curvature, improving the relationship between Euclidean
and geodesic distance~\citep{wang2026straightening}. RC-aux combines
multi-horizon predictive supervision with budget-conditioned
reachability correction~\citep{li2026rcaux}. Traj-LeWM learns a
goal-conditioned latent trajectory cost that shapes representations
during training and complements endpoint distance during
planning~\citep{huang2026trajlewm}. Trajectory Reachability Metrics
(TRM) learn a temporal pairwise cost from logged trajectories to
rank predicted endpoints against goals~\citep{li2026trm}.
These approaches show how temporal structure, reachability, and
intermediate-path information can enrich the interface between
prediction and action selection.

HAS focuses on the representation supplied to that interface.
A short action-conditioned rollout determines the candidate
endpoint, and a shared action-free map provides its temporal
extension before Euclidean goal distance is evaluated.
This use of local latent evolution places HAS alongside
action-free latent planning~\citep{masip2026ffjepa}, while its
endpoint-scoring role connects it to planning-oriented
costs~\citep{li2026trm,huang2026trajlewm}. The distinctive design
choice is to apply the extension after each candidate rollout
and set its depth from the remaining temporal gap. We next
formalize the time scales that define this operation.
...
the world model. Each training sample contains four encoded
...
We implement $F$ as a small residual MLP; its architecture and
the supervision diagram are given in
Appendix~\ref{app:implementation} and Figure~\ref{fig:method}.
...
An offline diagnostic complements these behavioral examples
by examining how Forward changes endpoint-to-goal distances
on evaluation-disjoint trajectory windows
(Appendix~\ref{app:analysis}). It compares encoder endpoints
with their temporal extensions and provides an additional
view of the learned scoring representation.
...
The scoring diagnostic examines how temporal extension
changes the distance between an intermediate representation
and a later goal. We sample $400$ offline trajectory windows
per task after excluding all episodes listed in the ten
evaluation-group manifests. Each window covers at least
$100$ environment steps and is encoded using the same
epoch-$10$ checkpoint as the control experiments.

For each window, we obtain encoder latents at the short
endpoint $H=25$ and goal offsets $o\in\{50,75,100\}$.
We compare the Euclidean distances
\[
d_{\mathrm{direct}}=\|z_H-z_g\|_2,
\qquad
d_{\mathrm{extended}}=\|F^k(z_H)-z_g\|_2,
\]
where $(k,o)\in\{(5,50),(10,75),(15,100)\}$.
These computations use observed endpoints encoded from the
dataset. They measure the behavior of the Forward
representation under temporal separation; candidate
rollout prediction and CEM selection are outside this
diagnostic.
...
distance range than direct endpoint comparisons
(Figure~\ref{fig:score}). In PushT, the two distributions
overlap more at the longest offset, alongside the greater
control difficulty observed at that offset.
The diagnostic complements the closed-loop results by
showing how the learned temporal map transforms the
quantity used for scoring. Its evidential scope is
representation-to-goal distance on observed windows,
rather than candidate-ranking accuracy.
